...
@interface RxWebViewController ()<UIWebViewDelegate,UINavigationControllerDelegate,UINavigationBarDelegate,NJKWebViewProgressDelegate,UIAlertViewDelegate,UIGestureRecognizerDelegate>
{
    NSString *lastUrlSring;
...
- (void)viewDidLoad{
    ...
    statusHeight = rectStatus.size.height;
    float navHeight;
    navHeight = self.navigationController.navigationBar.frame.size.height;
    upsideheight = statusHeight + navHeight;
//    [self loadWebView];
...
- (void)checkLoginState{
    if (_needLogin) {
        if ([NSUserDefaults boolForKey:IS_LOGIN]) {
            ...
            _urlStr = [NSString stringWithFormat:@"%@?s=%@",_urlStr,userinfo.loginID];
            [self loadUrl];
        }
        else
        {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"您还没有登录，是否登录后再浏览？" delegate:self cancelButtonTitle:@"先看看" otherButtonTitles:@"去登录", nil];
            alert.delegate = self;
            [alert show];
        }
    }
...
- (void)loadUrl{
    self.url = [NSURL URLWithString:_urlStr];
...
-(void)viewDidDisappear:(BOOL)animated{
    [_webView removeGestureRecognizer:self.swipePanGesture];
...
-(void)updateNavigationItems{
    ...
    if (self.webView.canGoBack)
    {
        ...
        self.navigationItem.leftBarButtonItems = @[negativeSpacer,self.customBackBarItem,self.closeButtonItem];
    }
    else{
        self.swipePanGesture.enabled = NO;
        self.navigationItem.leftBarButtonItems = @[negativeSpacer,self.customBackBarItem];
    }
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    self.title = theTitle;
    /**
     @author Sunc, 15-12-09 11:12:42
     
     完成loading后把url保存下来
     */
    lastUrlSring = _urlStr;
}
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        UIImage* backItemImage = [[UIImage imageNamed:@"back.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        UIImage* backItemHlImage = [[UIImage imageNamed:@"back.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
