...
{
    /**
     @author Sunc, 15-12-08 14:12:56
     
     这个地方注意window的关系，要好好补充这个些知识
     */
...
- (void)hideWithCompletion:( void (^) (BOOL finished) )completionBlock
{
	[UIView animateWithDuration:AGPhotoBrowserAnimationDuration
					 ...
					 completion:^(BOOL finished){
						 self.userInteractionEnabled = NO;
                         [_superController dismissViewControllerAnimated:YES completion:^{
...
- (UIButton *)doneButton
{
	if (!_doneButton) {
		...
		[_doneButton setTitle:NSLocalizedString(@"返回", @"Title for Done button") forState:UIControlStateNormal];
