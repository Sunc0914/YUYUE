...
@implementation ActivityHeaderPraiseView
{
    //参与btnbackview
...
- (void)setSubDetail:(NSDictionary *)dic{
    ...
    //这里的dic是整个result
    praiseUsers = [[NSMutableArray alloc]initWithArray:[dic objectForKey:@"praiseUserIds"]];
    motionDic = [NSDictionary dictionaryWithDictionary:[dic objectForKey:@"motion"]];
    praiseUsersNames = [[NSMutableArray alloc]initWithArray:[dic objectForKey:@"praiseUserNames"]];
    ...
    NSString *string = [NSString stringWithFormat:@"%@",[motionDic objectForKey:@"praiseCount"]];
    if (string.length == 0||[string isEqualToString:@"(null)"]) {
        string = @"0";
    }
    
    joinNumLabel.text = [NSString stringWithFormat:@"已点赞人数（%@）",string];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://m.yuti.cc/app/user/photo/%@",[praiseUsers objectAtIndex:indexPath.row+temi]]];
...
- (void)subscribeUserInfoClicked:(NSInteger)sender{
    ...
    userInfoVc.userID = [praiseUsers objectAtIndex:sender];
